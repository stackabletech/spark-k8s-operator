...
The backend is configured through the `lineage.connection`, which either inlines an OpenLineage
...
spec:
  lineage: # <1>
    ...
    # jobName: orders-by-nation # <4>
  ...
----
<1> Adding the `lineage` block enables OpenLineage event emission. Omit it entirely to leave OpenLineage off (the default), in which case the operator emits nothing OpenLineage-related and behaviour is unchanged.
<2> The OpenLineage backend connection. Use `inline` (shown here) to define the connection directly,
    ...
    Required. The connection selects a transport; `http` is currently the only one supported.
<3> Optional. The OpenLineage namespace events are reported under.
    Defaults to `default`.
<4> Optional. The stable OpenLineage job name (see <<job-name>>).

The `OpenLineageConnection` resource (`lineage.stackable.tech`) is a reusable definition of a
...
Set `spec.lineage.jobName` (or `spark.app.name`) to a stable value to keep all runs of the same logical job together.

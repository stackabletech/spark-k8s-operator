...
The secret with S3 credentials mus contain at least the following two fields:

* `accessKeyId` - the access key to the event log bucket.
* `secretAccessKey` - the secret key to the event log bucket.

Any other entries in the `stringData` field of the Secret are ignored by the operator.

...
<2> Artifact class
<3> S3 section, specifying the existing secret and config map
...
|`spec.s3.configMapName`
|Config map containing the keys `BUCKET_HOST`, `BUCKET_PORT` and `BUCKET_NAME`

|`spec.s3.credentialsSecretName`
|Secret containing the keys `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`

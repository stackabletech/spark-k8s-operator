...
NOTE: `spark-defaults.conf` is not required here, because the properties defined in {crd-ref}/spark.stackable.tech/v1alpha1/sparkhistoryserver{crd-ref-release}#sparkConf[`sparkConf` (SparkHistoryServer)] and {crd-ref}/spark.stackable.tech/v1alpha1/sparkapplication{crd-ref-release}#sparkConf[`sparkConf` (SparkApplication)] are already added to this file.
...
Usually, environment variables are configured in `envOverrides` or {crd-ref}/spark.stackable.tech/v1alpha1/sparkapplication{crd-ref-release}#env[`env` (SparkApplication)], but both options only allow static values to be set.
...
A key such as `MY-VAR` is rejected, because `export MY-VAR="..."` is not something the shell can execute. Please note that shell-readonly variables cannot be overridden.
...
In a SparkApplication, environment variables can also be defined with the {crd-ref}/spark.stackable.tech/v1alpha1/sparkapplication{crd-ref-release}#env[`env`] property for the job, driver and executor pods at once.

...
Apache Spark Connect is a remote procedure call (RPC) server that allows clients to run Spark applications on a remote cluster. Clients can connect to the Spark Connect server using a variety of programming languages, editors, IDEs without needing to install Spark locally.
...
<3> Additional package to install when starting the Connect server and executors.
...
* Integration with the history server.

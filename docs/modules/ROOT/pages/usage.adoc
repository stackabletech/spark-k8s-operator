...
* `s3`: configuration to connect to an S3 bucket.
** `configMapName`: the name of a config map containing the keys `BUCKET_HOST`, `BUCKET_PORT` and `BUCKET_NAME`.
** `credentialsSecretName`: the name of a secret containing the keys `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`.

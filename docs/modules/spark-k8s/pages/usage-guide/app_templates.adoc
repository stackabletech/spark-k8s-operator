...
NOTE: The `jvmArgumentOverrides` are an exception to the precedence rule above.
They are not overridden but applied in sequence: the overrides of the templates are applied first, in the order the templates are referenced, and the ones of the SparkApplication last.
An application can therefore also remove a JVM argument that one of its templates added.
